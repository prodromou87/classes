...
> myName  = "Vasiliki Papavasileiou"
> myEmail = "vpapavas@cs.ucsd.edu"
> mySID   = "A50059057"
> myPartber = "Akshay Balsubramani"
...
The function foldl is very slow because it uses lazy evaluation; in other words, 
it does not apply the input function f to anything at all until the whole list's arguments
are expanded out in one large thunk as an intermediate step. In contrast, foldl' uses 
strict evaluation, so it evaluates f sequentially as soon as it can. Therefore, it does 
not have to work with a massive thunk in memory before evaluating everything, simply a
constant-sized intermediate calculation.
...
> 
> tree = Bind 5 'a' (Bind 2 'b' (Bind 3 'c' NEmpty NEmpty) (Bind 4 'd' NEmpty NEmpty)) (Bind 6 'e' (Bind 7 'f' NEmpty NEmpty) NEmpty)   
>
>
...
solar system.
 ...
 planets = orbit mercury sun 2.0 2.0 0.2
...
> 			 (s `over` (translateB (xRadi, yRadi) (scaleb ((lift0 1) + ((lift0 $ 1-f)*(-1 * sin(time* lift0 freq)))) m)) )
 ...
 orbit m s freq x y = let xRadi = lift0 x*cos(time* lift0 freq)
			   yRadi = lift0 y*sin(time* lift0 freq)
		       in cond ( 0.0 >* sin (time*2)) ((translateB (xRadi, yRadi) m) `over` s) (s `over` (translateB (xRadi, yRadi) m))	
