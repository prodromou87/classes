---
title: Homework #3, Due Sunday, February 13th
---
...
3. Download the test files 
   [test.imp]($root/static/test.imp),
   [fact.imp]($root/static/fact.imp), 
   [abs.imp]($root/static/abs.imp), 
   [times.imp]($root/static/times.imp).
...
![Pong ScreenShot]($root/static/pong.png)
...
> play p1score p2score vel = 
>   lift1 picToGraphic $ walls `over` paddle1 `over` paddle2 `over` paint yellow $ translate (xpos, ypos) ball
>   where xvel = vel `stepAccum` xhit ->> negate
>         xpos = integral xvel
