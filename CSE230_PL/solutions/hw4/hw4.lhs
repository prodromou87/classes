...
> --type ESW a = ErrorT Value (StateT Store (Writer String)) a 
> 
> evalESW :: Statement -> ErrorT Value (StateT Store (Writer String)) ()
> --evalESW :: Statement -> WriterT String (StateT Store (Value)) ()  
> evalESW = evalS
...
>			       value <- evalE e;
>			       tell $ s ++ show value ++ "\n";
> --			       tell $ s ++ show (evalState (evalE e) state) ++ "\n";
>			       put state}
>				
> evalS (Throw e)	= do { state <- get;
>			       value <- evalE e;
>			       throwError $ value;
> --			       throwError $ (evalState (evalE2 e) state);
>			       put state}
>				
> evalS (Try s x h)	= do  
> --  state <- get
...
> --   let (a, b, c) = execute state s
> --  case b of Just a -> Assign x a
> --  execute state h
> 
>
...
> -- Additional hardwiring necessary to deal with the special case of negative numbers: 
>   if ((binary $ map sample1 (x:xs)) == 0) then ([low], borrow)
>                                         else (sub:subs, bout)
>                                         where (sub, b)     = halfsub (x, borrow)
>                                               (subs, bout) = bitSubtractor (b,xs)

The following is a helper function for bitSubtractor that subtracts a borrow bit from another bit.
...
> tnum  = [high, low, high, low, high, high] -- 53
> tnum2 = [low, high, low, high, low, low] -- 10
...
The following multiplier circuit would work better if the first input is greater than the second, 
but as we are not concerned with efficiency, here's the shorter version :) 
