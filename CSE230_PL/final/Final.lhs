...
assignment, email Pat at `nvazou+230@cs.ucsd.edu`.
...
> bstRotateR :: BST k v -> BST k v
> bstRotateR (Bind k v (Bind k' v' a b) c) = Bind k' v' a (Bind k v b c)
> bstRotateR t                             = t
> 
> bstRotateL :: BST k v -> BST k v
> bstRotateL (Bind k' v' a (Bind k v b c)) = Bind k v (Bind k' v' a b) c
> bstRotateL t                             = t
> 
> -- Left (x, dx, hx) means that we did not rebalance yet, and the subtree x
> -- has a delta of dx, and a height of hx
> -- Right x means we did balance yet, so no need to carry height and delta
> -- since only one balance is enough to keep the tree balanced after insert
> superbstInsertBal :: (Ord k) => k -> v -> BST k v ->
>   Either (BST k v, Integer, Integer) (BST k v) -- t, delta t, height t
> superbstInsertBal k v Emp = Left (Bind k v Emp Emp, 0, 1)
> superbstInsertBal k v (Bind k' v' l r)
>   -- here, since we replaced a key, we "did" balance by doing nothing!
>   | k == k'   = Right $ Bind k v l r
>   | k < k'    = case superbstInsertBal k v l of
>       Left (l', dl, hl) -> let (dr, hr) = deltaAndHeight r in
>                               balIns (hl - hr) l' dl hl r dr hr (Bind k' v')
>       Right l'          -> Right $ Bind k' v' l' r
>   | otherwise = case superbstInsertBal k v r of
>       Left (r', dr, hr) -> let (dl, hl) = deltaAndHeight l in
>                               balIns (hl - hr) l dl hl r' dr hr (Bind k' v')
>       Right r'          -> Right $ Bind k' v' l r'
>   where balIns dt l dl hl r dr hr binder
>           | dt > 1  && dl < 0  = Right $ bstRotateR $ binder (bstRotateL l) r
>           | dt > 1  && dl >= 0 = Right $ bstRotateR $ binder l r
>           | dt < -1 && dr > 0  = Right $ bstRotateL $ binder l (bstRotateR r)
>           | dt < -1 && dr <= 0 = Right $ bstRotateL $ binder l r
>           | otherwise          = Left (binder l r, dt, 1 + max hl hr)
>         deltaAndHeight (Bind _ _ l r) = (hl - hr, 1 + max hl hr)
>           where hl = height l
>                 hr = height r
>         deltaAndHeight Emp            = (0, 0)

> bstInsert :: (Ord k) => k -> v -> BST k v -> BST k v
> bstInsert k v = either (\(x,y,z) -> x) id . superbstInsertBal k v
...
> delta :: BST k v -> Integer
> delta Emp            = 0
> delta (Bind k v l r) = height l - height r
> 
> balance :: BST k v -> BST k v
> balance Emp    = Emp
> balance n@(Bind k v l r)
>   | delta n > 1  && delta l < 0  = bstRotateR $ Bind k v (bstRotateL l) r
>   | delta n > 1  && delta l >= 0 = bstRotateR n
>   | delta n < -1 && delta r > 0  = bstRotateL $ Bind k v l (bstRotateR r)
>   | delta n < -1 && delta r <= 0 = bstRotateL n
>   | otherwise                    = n
> 
> greatestKeyVal :: (Ord k) => BST k v -> Maybe (k,v)
> greatestKeyVal Emp              = Nothing
> greatestKeyVal (Bind k v _ Emp) = Just (k,v)
> greatestKeyVal (Bind k _ _ r)   = greatestKeyVal r
> 
...
>   | k == k'     = balance $ Bind kMax vkMax (bstDelete kMax l) r
>   | k < k'      = balance $ Bind k' v (bstDelete k l) r
>   | otherwise   = balance $ Bind k' v l (bstDelete k r)
>   where (kMax, vkMax) = fromJust $ greatestKeyVal l -- l /= Emp
...
>   if n == 0
>       then retry
>       else do writeTVar tv (tail l, n - 1)
>               return (head l)
> -- SOLUTION END --
